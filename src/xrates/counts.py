...
from xtenors.dates import *
from xtenors.units import *

from . import conventions

# ---------------------------------------------------------------

iterators = tenors.iterators
calendars = tenors.calendars
arithmetic = tenors.arithmetic
adjustments = tenors.adjustments
...
def pack_30_360_C(y1, m1, d1, y2, m2, d2):
    return (
        (d2 - d1)
        + (360 * (y2 - y1))
        + (30 * (m2 - m1))
...
def pack_30_360_py(y1, m1, d1, y2, m2, d2):
    return (
        (d2 - d1)
        + (360 * (y2 - y1))
        + (30 * (m2 - m1))
...
def day_count_n_30_360_US(ddt1, ddt2, feb1, feb2):
    y1, m1, d1 = unpack_date(ddt1)
    y2, m2, d2 = unpack_date(ddt2)
...
def day_count_n_30E_360_ISDA(ddt1, ddt2, feb1, feb2):
    y1, m1, d1 = unpack_date(ddt1)
    y2, m2, d2 = unpack_date(ddt2)
...
day_count = day_count_py
day_factor = day_factor_py
